...
function calculateSubtotal() {
	...
	for (let i = 0; i < ITEMS.length; i++) {
		let quantity = parseInt(document.getElementById(ITEMS[i] + "-quantity").value);
		let price = parseFloat(document.getElementById(ITEMS[i] + "-price").innerText);
...
function calculateSalesTax() {
	return roundMoney(calculateSubtotal() * getSalesTaxRateForState(document.getElementById("state-tax").value)); 
}
